...
class SumRep(Rep):
    ...
    def approximately_equivariant_projector(self, sv_w_func_dict=None, return_sv=False, **kwargs):
        """ Overrides default implementation with a more efficient version which decomposes the constraints
            across the sum."""
        Ps = {}
        if return_sv:
        ...
        for rep in self.reps:
            if return_sv:
                P, sv_w_dict = rep.approximately_equivariant_projector(sv_w_func_dict, return_sv, **kwargs)
                Ps[rep] = P
                sv_w_dict_combined.update(sv_w_dict)
            else:
                P = rep.approximately_equivariant_projector(sv_w_func_dict, return_sv, **kwargs)
                Ps[rep] = P
        multiplicities = self.reps.values()
        def lazy_P(array):
            return lazy_direct_matmat(array[self.perm],Ps.values(),multiplicities)[self.invperm]#[:,self.invperm]

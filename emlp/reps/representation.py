...
class Rep(object):
    ...
    def equivariant_basis(self, k=None):  
        """ Returns the basis corresponding to the smallest k singular values,
            and a null_space_loss: the sum of the squares of said singular values. 
            If k is None, it is automatically set to select sv's less than 1e-5."""
        if Scalar==self: return torch.ones((1,1)), 0
    ...
    def equivariant_projector(self, k=None):
        """ Computes the (lazy) projection matrix P=QQᵀ that projects to the equivariant basis."""
        Q, null_space_loss = self.equivariant_basis(k=k)
        Q_lazy = lazify(Q)
        P = Q_lazy@Q_lazy.H
    ...
    def __add__(self, other):
        ...
        if isinstance(other,int):
            ...
            return emlp.reps.product_sum_reps.SumRep(self,other,G=self.G)
        else:
...
def mul_reps(ra,rb:int):
    ...
    if (not hasattr(ra,'concrete')) or ra.concrete:
        return emlp.reps.product_sum_reps.SumRep(*(rb*[ra]),G=ra.G)
    else:
